        ...
        if type_ == 'removed':
            ...
            if isinstance(value_.get("old_value"), dict):
                result.append(f"Property '{path.strip('.')}' was updated. "
                              f"From [complex value] to {lower(value_.get('new_value'))}")
            elif isinstance(value_.get("new_value"), dict):
                result.append(f"Property '{path.strip('.')}' was updated. "
                              f"From {lower(value_.get('old_value'))} to [complex value]")
            else:
                result.append(f"Property '{path.strip('.')}' was updated. "
                              f"From {lower(value_.get('old_value'))} to {lower(value_.get('new_value'))}")
        elif type_ == 'nested':
